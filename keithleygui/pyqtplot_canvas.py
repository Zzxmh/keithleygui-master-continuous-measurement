...
class SweepDataPlot(pg.GraphicsView):
    ...
    def plot_xy(self, x_data, y_data, x_label="X", y_label="Y", title=""):
        """
        绘制一般的X-Y数据
        
        :param x_data: X轴数据列表或数组
        :param y_data: Y轴数据列表或数组
        :param x_label: X轴标签
        :param y_label: Y轴标签
        :param title: 图表标题
        """
        ...
        # 设置标题和标签
        self.setTitle(title)
        self.x_axis.setLabel(x_label, color="k", size="12pt")
        self.y_axis.setLabel(y_label, color="k", size="12pt")
        
        # 禁用对数模式
        self.p.setLogMode(x=False, y=False)
        
        # 绘制数据
        pen = fn.mkPen(color=COLORS[0], width=self.LW)
        self.p.plot(x_data, y_data, pen=pen)
